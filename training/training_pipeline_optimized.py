...
class OptimizedFlotationPipeline:
    ...
    def load_and_prepare_data(self):
        ...
        # Load data
        print("📊 Loading data...")
        data_path = Path('../data/HZL_RA4_Pb_Rougher_enhanced_clean.parquet')
        ...
        # Drop target column (timestamp is the index)
        exclude_cols = [target_col]
        X = df.drop(columns=exclude_cols).select_dtypes(include=[np.number])  # Keep only numeric features
        y = df[target_col]
        
        print(f"   Features: {X.shape[1]}")
        
        # Feature selection using mutual information (reduced for speed)
        # This helps identify the most relevant features for prediction
        print("🔍 Performing feature selection...")
        # Use more features for better performance
        max_features = min(200, X.shape[1])  # Increased back to 200 features
        ...
        sample_size = min(100000, len(X_selected))  # Increased to 100K for better performance
    ...
    def optimize_models(self, X_train, X_val, y_train, y_val):
        ...
        models_config = [
            ...
            ('ridge', Ridge(random_state=42), {
                'alpha': [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]  # Expanded regularization strength
            }),
            
            # Random Forest: Ensemble of decision trees
            ('rf', RandomForestRegressor(random_state=42, n_jobs=1), {  # Disable parallel processing on Windows
                'n_estimators': [100, 200, 300],      # Expanded parameter space
                'max_depth': [10, 15, 20, None],      # Expanded parameter space
            ...
            ('xgb', xgb.XGBRegressor(
                random_state=42, 
                n_jobs=1,  # Disable parallel processing on Windows
        ...
        for i, (name, model, params) in enumerate(models_config):
            ...
            if name == 'ridge':
                ...
                search = GridSearchCV(model, params, cv=3, scoring='r2', n_jobs=1)  # Disable parallel processing on Windows
            else:
                # Use RandomizedSearchCV for tree-based models (larger parameter space)
                search = RandomizedSearchCV(
                    model, params, n_iter=10, cv=3, scoring='r2', n_jobs=1, random_state=42  # Disable parallel processing on Windows
                )
    ...
    def save_best_model(self, results, best_model_name):
        ...
        model_path = f'../backend/trained_models/rf_optimized_model.pkl'
        
        # Create models directory if it doesn't exist
        Path('../backend/trained_models').mkdir(parents=True, exist_ok=True)
        ...
        metadata_path = f'../backend/trained_models/rf_metadata.pkl'
    ...
    def print_summary(self, results, best_model_name):
        ...
        print(f"\n📁 Generated Files:")
        print(f"   • Model: backend/trained_models/rf_optimized_model.pkl")
        print(f"   • Metadata: backend/trained_models/rf_metadata.pkl")
...
def main():
    ...
    enhanced_data_path = "HZL_RA4_Pb_Rougher_enhanced_clean.parquet"  # Use relative path
